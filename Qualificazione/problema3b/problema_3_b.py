...
def cosine_similarity(matrix1, matrix2):
    return scalar_product(matrix1, matrix2) / (
        np.sqrt(scalar_product(matrix1, matrix1)) * np.sqrt(scalar_product(matrix2, matrix2)))
...
if __name__ == '__main__':
    ...
    m1 = df[df.nome_squadra == 'Croatia'].pivot_table(values='frequenza_eventi', index='numero_cella_y',
                                                      columns='numero_cella_x')
    m2 = df[df.nome_squadra == 'France'].pivot_table(values='frequenza_eventi', index='numero_cella_y',
                                                     columns='numero_cella_x')

    sim_eventi = cosine_similarity(m1, m2)

    m1 = df[df.nome_squadra == 'Croatia'].pivot_table(values='frequenza_passaggi_accurati', index='numero_cella_y',
                                                      columns='numero_cella_x')
    m2 = df[df.nome_squadra == 'France'].pivot_table(values='frequenza_passaggi_accurati', index='numero_cella_y',
                                                     columns='numero_cella_x')

    sim_pass = cosine_similarity(m1, m2)

    m1 = df[df.nome_squadra == 'Croatia'].pivot_table(values='frequenza_tiri', index='numero_cella_y',
                                                      columns='numero_cella_x')
    m2 = df[df.nome_squadra == 'France'].pivot_table(values='frequenza_tiri', index='numero_cella_y',
                                                     columns='numero_cella_x')

    sim_tiri = cosine_similarity(m1, m2)

    m1 = df[df.nome_squadra == 'Croatia'].pivot_table(values='frequenza_falli_subiti', index='numero_cella_y',
                                                      columns='numero_cella_x')
    m2 = df[df.nome_squadra == 'France'].pivot_table(values='frequenza_falli_subiti', index='numero_cella_y',
                                                     columns='numero_cella_x')

    sim_falli = cosine_similarity(m1, m2)

    pd.DataFrame([[1, sim_eventi], [2, sim_pass], [3, sim_tiri], [4, sim_falli]],
                 columns=['tipo_griglia', 'similarity']).to_csv('problema_3_b.csv', index=False)
